 ...
 <div id="header">
	<div class="container">
		<div id="welcomeLine" class="row">
			<div class="span6">
				@if(empty(Auth::user()->name)) @else Welcome! <strong>{{Auth::user()->name}} @endif</strong></div>
			<div class="span6">
		  ...
		  <div class="navbar">
		    <div class="navbar-inner">
		      <div class="container">
		        ...
		        <a class="brand" href="#">Student kart</a>
		        <div class="nav-collapse">
		          <ul class="nav">
		            <li class="active"><a href="{{url('/')}}">Home</a></li>
@foreach($sections as $section)
@if(count($section['categories'])>0)
{{--if there is no category and sub cateory then main section will not shown--}}		            
		            <li class="dropdown">
		              <a href="#" class="dropdown-toggle" data-toggle="dropdown">{{$section['name']}}<b class="caret"></b></a>
		              <ul class="dropdown-menu">
@foreach($section['categories'] as $category)		              	                      <li class="divider"></li>
		                <li class="nav-header"><a href="{{$category['url']}}">{{$category['category_name']}}</a></li>
@foreach($category['sub_categories'] as $sub)
		                <li><a href="{{$sub['url']}}">{{$sub['category_name']}}</a></li>
@endforeach	
@endforeach	                
		              </ul>
		            </li>
